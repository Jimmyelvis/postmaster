...
const Modal = ({ children, selector, overlayColor, modalTarget, modalOrigin, delay, classes}) => {
  ...
  const isModalOpen = useSelector((state) => state.dashBoardUi.isModalOpen);
  const origin = useSelector((state) => state.dashBoardUi.origin);
   ...
   * reducer, matches the value of the modalOrigin prop that is passed
